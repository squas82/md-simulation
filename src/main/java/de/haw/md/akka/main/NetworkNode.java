...
public class NetworkNode extends UntypedActor {
	...
	private Map<String, RecievedPackages> recievedPackagesMap = new HashMap<>();
	...
	private void proofRecievedPackagesMap() {
		...
		for (String packageID : recievedPackagesMap.keySet()) {
			RecievedPackages recievedPackage = recievedPackagesMap.get(packageID);
			if (!recievedPackage.isRecieved() && recievedPackage.getTimestamp().plusSeconds(1).isBefore(LocalDateTime.now())) {
				if (recievedPackage.getSendingAttempts() < StaticValues.SENDING_ATTEMPTS) {
					recievedPackage.setSendingAttempts(recievedPackage.getSendingAttempts() + 1);
					handleNetworkMsgHelper(recievedPackage.getMsgModel());
				} else if (recievedPackage.getSendingAttempts() < StaticValues.MAX_ATTEMPTS_TO_TIMEOUT) {
					recievedPackage.setSendingAttempts(recievedPackage.getSendingAttempts() + 1);
					routes.remove(recievedPackage.getMsgModel().getDst());
					handleNetworkMsgHelper(recievedPackage.getMsgModel());
				} else {
		...
		if (!proofRecieved(networkMsgResponseModel.getId())) {
			if (nodeID.compareToIgnoreCase(networkMsgResponseModel.getDst()) == 0 && nodeID.compareToIgnoreCase(networkMsgResponseModel.getSrc()) == 0) {
				...
				if (recievedPackagesMap.containsKey(networkMsgResponseModel.getOriginalId())) {
					RecievedPackages recievedPackeges = recievedPackagesMap.get(networkMsgResponseModel.getOriginalId());
					recievedPackeges.setRecieved(true);
//					System.out.println("Package successful transmitted, PackegeID: " + recievedPackeges.getMsgModel().getId());
				}
...
//						+ networkMsgModel.getDst());
			...
			if (nodeID.compareToIgnoreCase(networkMsgModel.getSrc()) == 0) {
				...
				if (nodeID.compareToIgnoreCase(networkMsgModel.getSrc()) == 0 && nodeID.compareToIgnoreCase(networkMsgModel.getOriginalSrc()) == 0) {
					recievedPackagesMap.put(networkMsgModel.getId(), new RecievedPackages(networkMsgModel, false, LocalDateTime.now()));
				}
